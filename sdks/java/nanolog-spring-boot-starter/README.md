# NanoLog Spring Boot Starter

高性能、轻量级的日志收集 SDK，将 Logback 日志异步发送到 NanoLog Server。

[![Maven Central](https://img.shields.io/maven-central/v/tech.coffers/nanolog-spring-boot-starter)](https://central.sonatype.com/artifact/tech.coffers/nanolog-spring-boot-starter)
[![License](https://img.shields.io/badge/License-MIT-blue.svg)](LICENSE)

## ✨ 特性

- 🚀 **异步批量发送** - 后台 Worker 线程批量处理，零阻塞主线程
- 🔄 **指数退避重试** - 发送失败自动重试 (100ms → 200ms → 400ms)
- 💾 **本地降级存储** - 网络不可用时写入本地文件，服务恢复后自动重发
- ⚙️ **Spring Boot 自动配置** - 开箱即用，零代码集成
- 📊 **轻量高效** - 无额外依赖，内存占用极低

---

## 🏗️ 架构设计

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                              Your Application                                │
│                                                                              │
│     log.info()         log.error()         log.warn()                       │
│         │                   │                   │                            │
└─────────┼───────────────────┼───────────────────┼────────────────────────────┘
          │                   │                   │
          └───────────────────┼───────────────────┘
                              ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                              NanoLog SDK                                     │
│  ┌──────────────────────────────────────────────────────────────────────┐   │
│  │                    BlockingQueue (10000)                              │   │
│  └──────────────────────────────────────────────────────────────────────┘   │
│                              │                                               │
│              ┌───────────────┴───────────────┐                              │
│              ▼                               ▼                              │
│  ┌─────────────────────────┐    ┌─────────────────────────┐                │
│  │     Worker Thread       │    │    Recovery Thread      │                │
│  │  ┌───────────────────┐  │    │      (每分钟)            │                │
│  │  │  Batch Builder    │  │    │  ┌───────────────────┐  │                │
│  │  └─────────┬─────────┘  │    │  │ 读取 fallback.log │  │                │
│  │            ▼            │    │  └─────────┬─────────┘  │                │
│  │  ┌───────────────────┐  │    │            ▼            │                │
│  │  │   Retry Logic     │  │    │  ┌───────────────────┐  │                │
│  │  │ 100→200→400ms     │  │    │  │   Ping Server     │  │                │
│  │  └─────────┬─────────┘  │    │  └─────────┬─────────┘  │                │
│  └────────────┼────────────┘    │            ▼            │                │
│               │                 │  ┌───────────────────┐  │                │
│               │                 │  │   重发日志         │  │                │
│               │                 │  └─────────┬─────────┘  │                │
│               │                 └────────────┼────────────┘                │
│               │                              │                              │
│               │ 3次失败                       │                              │
│               ▼                              │                              │
│  ┌─────────────────────────┐                 │                              │
│  │     Fallback File       │◄────────────────┘                              │
│  │ /tmp/nanolog/fallback/  │                                                │
│  └─────────────────────────┘                                                │
└──────────────────────────────────┬──────────────────────────────────────────┘
                                   │ 成功
                                   ▼
                    ┌─────────────────────────┐
                    │     NanoLog Server      │
                    │  ┌───────────────────┐  │
                    │  │   /api/ingest     │  │
                    │  │   /api/ping       │  │
                    │  └───────────────────┘  │
                    └─────────────────────────┘
```

### 发送流程

```
Application              Queue              Worker             Server           Fallback
    │                      │                  │                   │                 │
    │  log.info("msg")     │                  │                   │                 │
    │─────────────────────>│                  │                   │                 │
    │                      │   poll(100ms)    │                   │                 │
    │                      │<─────────────────│                   │                 │
    │                      │   日志事件        │                   │                 │
    │                      │─────────────────>│                   │                 │
    │                      │                  │                   │                 │
    │                      │                  │ POST /api/ingest  │                 │
    │                      │                  │──────────────────>│                 │
    │                      │                  │                   │                 │
    │                      │                  │   [成功] 200 OK   │                 │
    │                      │                  │<──────────────────│                 │
    │                      │                  │                   │                 │
    │                      │                  │   [失败] 等待100ms │                 │
    │                      │                  │──────────────────>│                 │
    │                      │                  │   [失败] 等待200ms │                 │
    │                      │                  │──────────────────>│                 │
    │                      │                  │   [失败] 等待400ms │                 │
    │                      │                  │──────────────────>│                 │
    │                      │                  │                   │                 │
    │                      │                  │  [3次失败] 写入    │                 │
    │                      │                  │───────────────────────────────────>│
    │                      │                  │                   │                 │
```

---

## 📦 安装

### Maven

```xml
<dependency>
    <groupId>tech.coffers</groupId>
    <artifactId>nanolog-spring-boot-starter</artifactId>
    <version>0.1.0</version>
</dependency>
```

### Gradle

```groovy
implementation 'tech.coffers:nanolog-spring-boot-starter:0.1.0'
```

---

## ⚙️ 配置

在 `application.yml` 中添加配置：

```yaml
nanolog:
  # 基础配置
  enabled: true                              # 是否启用 (默认: true)
  server-url: http://localhost:8080          # NanoLog Server 地址
  service: ${spring.application.name}        # 服务名称

  # 性能调优
  batch-size: 100                            # 批量大小 (默认: 100)
  flush-interval-ms: 1000                    # 刷新间隔毫秒 (默认: 1000)

  # 容错配置
  enable-fallback: true                      # 启用本地降级 (默认: true)
  fallback-path: /tmp/nanolog/fallback       # 降级文件路径
```

### 配置项说明

| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| `nanolog.enabled` | boolean | `true` | 是否启用 NanoLog |
| `nanolog.server-url` | string | `http://localhost:8080` | NanoLog Server 地址 |
| `nanolog.service` | string | `default` | 服务标识，用于区分日志来源 |
| `nanolog.batch-size` | int | `100` | 每批发送的日志条数 |
| `nanolog.flush-interval-ms` | long | `1000` | 最大刷新间隔 (ms) |
| `nanolog.enable-fallback` | boolean | `true` | 是否启用本地降级存储 |
| `nanolog.fallback-path` | string | `/tmp/nanolog/fallback` | 降级文件存储目录 |

---

## 🚀 使用示例

### 基础使用

SDK 会自动注册到 ROOT Logger，无需任何代码修改：

```java
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

@RestController
public class UserController {

    private static final Logger log = LoggerFactory.getLogger(UserController.class);

    @GetMapping("/users/{id}")
    public User getUser(@PathVariable Long id) {
        log.info("Fetching user with id: {}", id);
        
        try {
            User user = userService.findById(id);
            log.debug("User found: {}", user.getName());
            return user;
        } catch (Exception e) {
            log.error("Failed to fetch user: {}", id, e);
            throw e;
        }
    }
}
```

### 生产环境配置示例

```yaml
# application-prod.yml
nanolog:
  enabled: true
  server-url: http://nanolog.internal.example.com:8080
  service: user-service
  batch-size: 200                    # 生产环境可适当增大批次
  flush-interval-ms: 500             # 缩短刷新间隔，降低延迟
  enable-fallback: true
  fallback-path: /var/log/nanolog/fallback  # 使用持久化目录
```

### 禁用 NanoLog

在特定环境（如本地开发）禁用：

```yaml
# application-local.yml
nanolog:
  enabled: false
```

---

## 🔧 容错机制

### 指数退避重试

当发送失败时，SDK 会按以下策略重试：

| 重试次数 | 等待时间 | 累计最大耗时 |
|----------|----------|--------------|
| 第 1 次  | 100ms    | 100ms        |
| 第 2 次  | 200ms    | 300ms        |
| 第 3 次  | 400ms    | 700ms        |

### 本地降级

如果 3 次重试全部失败，日志批次将写入本地文件：

```
/tmp/nanolog/fallback/fallback.log
```

文件格式为每行一个 JSON 数组：

```json
[{"timestamp":1735707600000000000,"level":"1","service":"my-service","host":"server-01","message":"User login"}]
[{"timestamp":1735707601000000000,"level":"3","service":"my-service","host":"server-01","message":"DB timeout"}]
```

### 自动恢复

后台 Recovery 线程每 **60 秒** 执行一次：

1. 检查 `fallback.log` 是否存在
2. 调用 `/api/ping` 探测 Server 可用性
3. 如果可用，读取并重发所有日志
4. 发送成功后删除本地文件

---

## 📊 性能参考

| 指标 | 数值 |
|------|------|
| 队列容量 | 10,000 条 |
| 内存占用 | < 10MB |
| 吞吐量 | 50,000+ logs/sec |
| 批量延迟 | < 1 秒 (可配置) |

---

## 🔗 相关链接

- [NanoLog Server](https://github.com/coffersTech/nanolog)
- [问题反馈](https://github.com/coffersTech/nanolog/issues)

---

## 📄 License

MIT License © 2024 Coffers Tech
